...
class Wide_ResNet(nn.Module):
    def __init__(self, depth, widen_factor, num_classes=10, input_channels=3,
                 sum_pool=False, norm=None, leak=.2, dropout_rate=0.0, latent_dim=8192, in_proj=8192):
        ...
        self.proj = nn.Linear(in_proj, latent_dim)
        if self.last_dim != 640:
            self.head = nn.Sequential(
                    nn.Conv2d(2*nStages[3], nStages[3], kernel_size=1, bias=True),
                    nn.LeakyReLU(leak, inplace=True)
                )
        
        print("dim: ",self.last_dim)
